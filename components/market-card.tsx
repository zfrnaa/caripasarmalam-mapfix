  ...
  return (
    <Card className="overflow-hidden hover:shadow-lg transition-shadow h-full flex flex-col">
      ...
      <CardContent className="flex flex-col h-full">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {orderedSchedule.map((sch) => {
            const times = sch.times
              .map((s) => `${s.start}–${s.end}`)
              .join(', ');
            const dayLabel = sch.days
              .map((d) => getLocalizedDayFromCode(d))
              .join(', ');
        ...
        {(isPositiveNumber(market.total_shop) ||
          isPositiveNumber(market.area_m2)) && (
          <p className="text-sm text-muted-foreground mb-4">
            {[
              isPositiveNumber(market.total_shop)
                ? `${market.total_shop} ${t.totalStalls.toLowerCase()}`
                : null,
              isPositiveNumber(market.area_m2)
                ? `${formatArea(market.area_m2)}`
                : null
            ]
              .filter(Boolean)
              .join(' • ')}
